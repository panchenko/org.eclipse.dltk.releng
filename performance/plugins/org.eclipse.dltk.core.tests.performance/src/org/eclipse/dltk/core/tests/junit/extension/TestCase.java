...
public class TestCase extends PerformanceTestCase {
	 ...
	 * 
	 * @see #buildTestsList(Class, int, long) <br>
	 ...
	 *      <li>{@link #ALPHA_REVERSE_SORT}: alpha reverse order (ie. descending
	 *      )</li>
	 *      <li>{@link #RANDOM_ORDER_JDT}: random order using JDT/Core current
	 *      version as seed</li>
	 *      <li>{@link #RANDOM_ORDER_TIME}: random order using current time as
	 *      seed (used time value is displayed in console)</li>
	 *      <li>other values: random order using given <code>long</code> value
	 *      as seed</li>
	 *      </ul>
	 *      This value is initialized with <code>"ordering"</code> system
	 *      property.
	 */
	...
	static {
		...
		try {
			...
			try {
				int kind = Integer
						.parseInt(System.getProperty("ordering", "0"));
				switch (kind) {
				case NO_ORDER:
					break;
				case ALPHABETICAL_SORT:
					ordering = kind;
					System.err
							.println("Note that tests will be run sorted using alphabetical order...");
					break;
				case ALPHA_REVERSE_SORT:
					ordering = kind;
					System.err
							.println("Note that tests will be run sorted using alphabetical reverse order...");
					break;
				case RANDOM_ORDER_TIME:
					ordering = System.currentTimeMillis();
					System.err
							.println("Note that tests will be run in random order using seed="
									+ ordering + " (ie. current time)");
					break;
				default:
					ordering = seed;
					System.err
				...
				// ordering value is over int range but is a valid long => keep
				// the value
				ordering = seed;
				System.err
						.println("Note that tests will be run in random order using seed="
			...
			System.err
					.println("Only integer or long values are allowed for 'ordering' system property: "
							+ System.getProperty("ordering", "0")
							+ " is not valid ! => no order will be finally used...");
			ordering = NO_ORDER;
		}
		ORDERING = ordering;
	}
	...
	private static final NumberFormat DIGIT_FORMAT = NumberFormat
			.getNumberInstance();

	/*
	 * Static initializer for memory trace. This functionality is activated
	 * using system property "storeMemory". Here's possible format for this
	 * property: -DstoreMemory=<file name without
	 * extension>[,all][,gc][,dir=<directory name>] <file name>: name of the
	 * file where memory data will be stored optional parameters: all: flag to
	 * store memory data for all tests. If not specified, then data will be
	 * stored only per test suite gc: flag to run garbage collection before each
	 * test or test suite (depending of "all" parameter) dir=<directory name>:
	 * specify directory where to put the file. Default is the directory
	 * specified in 'user.home' property Example:
	...
	static {
		...
		if (storeMemory != null) {
			...
			if (index > 0) {
				StringTokenizer parameters = new StringTokenizer(storeMemory
						.substring(storeMemory.indexOf(',') + 1), ",");
	...
	/**
	 * Flag telling whether test execution must stop on failure or not. Default
	 * is true;
	 */
	...
	public static void assertStringEquals(String message, String expected,
			String actual, boolean showLineSeparators) {
	 ...
	 * {@link #abortOnFailure} has been set to <code>true</code>. Otherwise, the
	 * thrown exception {@link AssertionFailedError} is caught and its message
	 * is only displayed in the console hence producing no JUnit failure.
	 */
	...
	protected void assumeEquals(String msg, String expected, String actual) {
		try {
			...
			System.out.println("Failure while running test "
					+ Performance.getDefault().getDefaultScenarioId(this)
					+ "!!!");
	...
	private void printAssertionFailure(AssertionFailedError afe) {
		System.out
				.println("\n!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!!---!");
		System.out.println("Caught assertion failure while running test "
				+ getName() + ":");
	...
	/**
	 * Build a list of methods to run for a test suite. There's no recursion in
	 * given class hierarchy, methods are only public method starting with
	 * "test" of it.
	 * <p>
	 * </p>
	 ...
	 * <ul>
	 * <li>{@link #TESTS_PREFIX}: only methods starting with this prefix (after
	 * "test" of course) will be put in test suite.</li>
	 * <li>{@link #TESTS_NAMES}: only methods with these names will be put in
	 * test suite.</li>
	 * <li>{@link #TESTS_NUMBERS}: only methods including these numbers will be
	 * put in test suite.<br>
	 * For example, <code>TESTS_NUMBERS = new int[] { 10, 100, 125678 };</code>
	 * will put <code>test010()</code>, <code>test100()</code> and
	 * <code>testBug125678()</code> methods in test suite.</li>
	 * <li>{@link #TESTS_RANGE}: only methods which numbers are between first
	 * and second value of this int array will be put in the suite. For example:
	 * <code>TESTS_RANGE = new int[] { 10, 12 };</code> will put
	 * <code>test010()</code>, <code>test011()</code> and <code>test012()</code>
	 * methods in test suite.<br>
	 * Note that -1 will clean min or max value, for example
	 * <code>TESTS_RANGE = new int[] { 10, -1 };</code> will put all methods
	 * after <code>test010()</code> in the test suite.</li>
	 * </ul>
	 * <p>
	 * </p>
	 * 2) testONLY_ methods<br>
	 * As static variables needs a static initializer usually put at the
	 * beginning of the test suite, it could be a little be boring while adding
	 * tests at the end of the file to modify this static initializer. One
	 * solution to avoid this was to introduced specific methods name which will
	 * be only executed when test suite is run alone. For example:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * This test suite will have only test "testONLY_100" put in test suite
	 * while running it.
	 * 
	 * Note that these 2 mechanisms should be reset while executing "global"
	 ...
	 * </pre>
	 * 
	 * This will insure you that all tests will be put in TestAll test suite,
	 * even if static variables values are set or some methods start as
	 * testONLY_...
	 * 
	...
	public static List buildTestsList(Class evaluationTestClass) {
		return buildTestsList(evaluationTestClass, 0/* only one level */,
				ORDERING);
	}

	/**
	 * Build a list of methods to run for a test suite. <br>
	 * Differ from {@link #buildTestsList(Class)} in the fact that one can
	 * specify level of recursion in hierarchy to find additional tests.
	 * 
	 * @param evaluationTestClass
	 *            the test suite class
	 * @param inheritedDepth
	 *            level of recursion in top-level hierarchy to find other tests
	...
	/**
	 * Build a list of methods to run for a test suite. <br>
	 * This list may be ordered in different ways using {@link #ORDERING}. <br>
	 * Example
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * Returned list will have 5 tests if inheritedDepth is equals to 1 instead
	 * of 3 if it was 0 as while calling by {@link #buildTestsList(Class)}.
	 * 
	 * @see #buildTestsList(Class) for complete explanation of subsets
	 *      mechanisms.
	 * 
	 * @param evaluationTestClass
	 *            the test suite class
	 ...
	 *            level of recursion in top-level hierarchy to find other tests
	 * @param ordering
	 *            kind of sort use for the list (see {@link #ORDERING} for
	 *            possible values)
	 * @return a {@link List list } of {@link Test tests}
	 */
	public static List buildTestsList(Class evaluationTestClass,
			int inheritedDepth, long ordering) {
		...
		for (int i = 0; i < inheritedDepth
				&& !isAbstract(evaluationTestSuperclass.getModifiers()); i++) {
			Method[] superMethods = evaluationTestSuperclass
					.getDeclaredMethods();
			Method[] mergedMethods = new Method[methods.length
					+ superMethods.length];
			System.arraycopy(superMethods, 0, mergedMethods, 0,
					superMethods.length);
			System.arraycopy(methods, 0, mergedMethods, superMethods.length,
					methods.length);
		...
		nextMethod: for (int m = 0, max = methods.length; m < max; m++) {
			...
			if (isPublic(modifiers) && !isStatic(modifiers)) {
				...
				if (methName.startsWith(METHOD_PREFIX)) {
					...
					boolean isOnly = RUN_ONLY_ID != null
							&& methName.substring(methodPrefixLength)
									.startsWith(RUN_ONLY_ID);
					if (isOnly) {
					...
					if (TESTS_PREFIX == null && TESTS_NAMES == null
							&& TESTS_NUMBERS == null && TESTS_RANGE == null) {
					...
					if (TESTS_PREFIX == null
							|| methName.startsWith(TESTS_PREFIX)) {
						int numStart = TESTS_PREFIX == null ? methodPrefixLength
								: TESTS_PREFIX.length();
						// tests names subset
						if (TESTS_NAMES != null) {
						...
						if (numStart < length) {
							...
							while (numStart < length
									&& !Character.isDigit(methName
											.charAt(numStart)))
								numStart++; // skip to first digit
							while (numStart < length
									&& methName.charAt(numStart) == '0')
								numStart++; // skip to first non-nul digit
							int n = numStart;
							while (n < length
									&& Character.isDigit(methName.charAt(n)))
								n++; // skip to next non-digit
							if (n > numStart && n <= length) {
								try {
									int num = Integer.parseInt(methName
											.substring(numStart, n));
									// tests numbers subset
									if (TESTS_NUMBERS != null
											&& !testNames.contains(methName)) {
									...
									// tests range subset
									if (TESTS_RANGE != null
											&& TESTS_RANGE.length == 2
											&& !testNames.contains(methName)) {
										if ((TESTS_RANGE[0] == -1 || num >= TESTS_RANGE[0])
												&& (TESTS_RANGE[1] == -1 || num <= TESTS_RANGE[1])) {
											testNames.add(methName);
											continue nextMethod;
										}
									}
						...
						if (TESTS_NAMES == null && TESTS_NUMBERS == null
								&& TESTS_RANGE == null) {
							if (!testNames.contains(methName)) {
								testNames.add(methName);
							}
		...
		while (iterator.hasNext()) {
			...
			try {
				...
				System.err.println("Method " + testName
						+ " removed from suite due to exception: "
						+ e.getMessage());
			}
		}
		return tests;
	...
	private static boolean isAbstract(int modifiers) {
		return Modifier.isAbstract(modifiers);
	}

	/**
	 * Build a test suite with all tests computed from public methods starting
	 * with "test" found in the given test class. Test suite name is the name of
	 * the given test class.
	 * 
	 * Note that this lis maybe reduced using some mechanisms detailed in
	 * {@link #buildTestsList(Class)} method.
	 * 
	 * @param evaluationTestClass
	...
	/**
	 * Build a test suite with all tests computed from public methods starting
	 * with "test" found in the given test class. Test suite name is the given
	 * name.
	 * 
	 * Note that this lis maybe reduced using some mechanisms detailed in
	 * {@link #buildTestsList(Class)} method.
	...
	public static Test buildTestSuite(Class evaluationTestClass,
			String suiteName) {
		TestSuite suite = new TestSuite(suiteName == null ? evaluationTestClass
				.getName() : suiteName);
	...
	private static File createMemLogFile() {
		...
		try {
			...
			if (stream != null) {
				...
				stream.close();
				System.out
						.println("Log file " + logFile.getPath() + " opened.");
				return logFile;
			} else {
			...
			if (logDir.mkdir()) {
				...
				System.err
						.println("Cannot create "
								+ logDir
								+ " as its parent does not exist. Log files will NOT be written!");
			}
		}
		return false;
	}
	...
	/**
	 * Clean test before run it. Currently, clean only performs a gc.
	 */
	...
	protected void setUp() throws Exception {
		...
		if (isFirstTestRun || CURRENT_CLASS != getClass()) {
			...
			CURRENT_CLASS_NAME = CURRENT_CLASS_NAME.substring(
					CURRENT_CLASS_NAME.indexOf(".tests.") + 7,
		...
		if (STORE_MEMORY != null && MEM_LOG_FILE != null) {
			...
			if (ALL_TESTS_LOG && MEM_LOG_FILE.exists()) {
				...
				if (isFirstTestRun) {
					long total = Runtime.getRuntime().totalMemory();
					long used = total - Runtime.getRuntime().freeMemory();
					System.out.println("	already used while starting: "
							+ format(used));
				}
			}
		}
