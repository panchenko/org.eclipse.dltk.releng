...
import org.eclipse.dltk.core.tests.junit.extension.PerformanceTestSuite;
...
public class AllPerformanceTests extends junit.framework.TestCase {
	...
	final static boolean ADD = System.getProperty("add", "false")
			.equals("true");
	...
	public static Class[] getAllTestClasses() {
		return new Class[] {
				AllDLTKTests.class,
				FullSourceWorkspaceSearchTests.class, // run this
				// test

				// first to
				// be sure that indexing
				// is finished
	...
	/**
	 * Build test suite. All classes suite method are called and bundle to main
	 * test suite.
	 */
	public static Test suite() {
		init();

		PerformanceTestSuite perfSuite = new PerformanceTestSuite(
				AllPerformanceTests.class.getName());
		...
		if (TestCase.TESTS_NAMES != null) {
			...
			buffer
					.append("	- following subset is still defined and may alter tests order:\n");
		...
		tooMuch = false;
		if (totalMem < (TOTAL_MEM * 0.98)
				|| (tooMuch = totalMem > (TOTAL_MEM * 1.02))) {
			if (buffer == null)
				buffer = new StringBuffer(
						"WARNING: Performance tests results may be invalid !!!\n");
			buffer.append("	- ");
		...
		if (RUN_ID != null) {
			...
			int count = 0;
			for (int i = 0; i < length; i++) {
				String name = FullSourceWorkspaceTests
						.suiteTypeShortName(testSuites[i]);
				if (RUN_ID.indexOf(name.charAt(0)) >= 0) {
					subSetSuites[count++] = testSuites[i];
				}
			}
			System.arraycopy(subSetSuites, 0, testSuites = new Class[count], 0,
					count);
			length = count;
		}
			...
			for (int i = 0; i < length; i++) {
				...
				if (name != null) {
					...
					if (suitesAcronym.indexOf(firstChar) >= 0) {
						System.out
								.println("WARNING: Duplicate letter in RUN_ID for test suite: "
										+ name);
					}
	...
	public static void init() {
		...
		// "org.eclipse.dltk.core.tests.performance:org.eclipse.dltk.core.tests.performance.util.DLTKCorePerformanceMeterFactory"
		// );
		// System.setProperty("logDir", "/home/dltk/perfomance");
	}
}
